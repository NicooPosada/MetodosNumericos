...
int main() {
    ...
    if (!esDiagonalmenteDominante(A)) {
        cout << "⚠️ Advertencia: La matriz NO es diagonalmente dominante." << endl;
    ...
    do {
        ...
        // Calcular error (norma euclidea de la diferencia)
        error = 0;
        for (int i = 0; i < n; i++) {
    ...
    // Calculo del residuo r = A·x - b
    vector<double> residuo(n, 0.0);
    for (int i = 0; i < n; i++) {
        double suma = 0;
        for (int j = 0; j < n; j++) {
            suma += A[i][j] * Xn[j];
